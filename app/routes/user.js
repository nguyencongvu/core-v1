...
var routePath = '/admin/users';  //--edit here -- routePath+'/:id'
...
function searchMiddle(req, res, next){
    ...
    if (    
            !searchValue &&
            !inString(urlPath,"users/report") &&
            !inString(urlPath,"users/excel") && 
            !inString(urlPath,"users/json") &&
            !inString(urlPath,"users/import") &&
            !inString(urlPath,"users/jsonmap")
        )
    { 
...
//     res.render(modelName+'/cards',searchResults); //view dang card
// });
...
//-- import 
router.get(routePath+'/import', function(req, res) {
    var results = { //-- to view
            title: 'Import ' + modelName,
            user: req.user,
        } 
    res.render(modelName+'/import',results);
});


//-- get list by Type
router.get(routePath+"/jsontype", function(req, res) {
    var ptype = req.param('type');

    Model.find({type: ptype}, function(err, data) {
        if (err) {
          return res.send(err);
        }
        res.json(data);
...
//-- GET formUpdateOrder -- khong dung 
...
//-- PUT care return JSON  -- khong dung 
...
router.post(routePath+'/:id/inlineEdit', function(req,res){
    
    // http://stackoverflow.com/questions/17039018/how-to-use-a-variable-as-a-field-name-in-mongodb-native-findone    
    var id = req.params.id; 

    var column = req.query.column.trim(); 
    var editVal = req.query.editval.trim();
...
//-- ko hieu sao khong new route.get o day duoc!!???
router.post(routePath+"/importsave", function(req,res){
    var data;
    data = req.body;
    console.log(data);
